...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if context is not None:
            # Handle both 4D (image) and 3D (sequence) context
            if context.dim() == 4:
                context = rearrange(context, 'b c h w -> b (h w) c')
            # If context.dim() == 3, it's already in sequence format [b n c]
...
class BasicTransformerBlock(nn.Module):
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=False):
        ...
        self.checkpoint = checkpoint  # Changed default to False to avoid BFloat16 issues
