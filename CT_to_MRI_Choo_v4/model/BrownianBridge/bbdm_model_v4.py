"""
Brownian Bridge Diffusion Model v4 — UQ edition.

Extends bbdm_model_v2 (ISTA-capable Brownian Bridge model) with a single
UQ-enabling change: the `dropout_rate` parameter is threaded through
BrownianBridgeModel.__init__() into UNetModel(dropout=dropout_rate).

Default dropout_rate=0.0  →  identical behaviour to v2/v3 (no dropout).
Non-zero dropout_rate      →  MC Dropout is available at inference by calling
                              `enable_mc_dropout(model)` from utils.mc_dropout.

Core algorithmic logic (Brownian Bridge schedule, forward/reverse diffusion,
ISTA) is UNCHANGED from v2.

Changes vs bbdm_model_v2:
  - BrownianBridgeModel.__init__: added `dropout_rate: float = 0.0` param.
  - UNetModel instantiation: `dropout=self.dropout_rate` added.
  - Docstring updated.
"""
import torch
...
class BrownianBridgeModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 # UQ: dropout rate for MC Dropout (default 0.0 = no dropout)
        ...
        # dropout_rate=0.0 (default) → identical to v2/v3 behaviour.
        # dropout_rate>0.0 → enables stochastic dropout in ResBlock.out_layers,
        # which can be kept active during inference via enable_mc_dropout().
        self.denoise_fn = UNetModel(
            ...
            dropout=self.dropout_rate,           # <-- v4 UQ addition
