...
v4: UQ edition — adds MC Dropout and Deep Ensemble support.

Changes from v3 (train_bbdm_ct2mri_wandb_v3.py):
  1. [Model]   Imports BrownianBridgeModel from bbdm_model_v4, which
               threads dropout_rate into UNetModel(dropout=dropout_rate).
  2. [Args]    New: --dropout_rate (float, default 0.0).
               At 0.0 behaviour is identical to v3 (no dropout applied).
  3. [Tags]    WandB tags updated to v4 / UQ.

Everything else — dataset, MR histogram, EMA, optimizer, checkpoint format,
and the core Brownian Bridge algorithm — is UNCHANGED from v3.

UQ notes:
  - MC Dropout : train once with --dropout_rate 0.05.
                 At inference call enable_mc_dropout(model) so dropout
                 stays active in model.eval() mode.
  - Deep Ensemble: train N times with different --seed via train_ensemble_v4.sh.
                 Each run produces a separate checkpoint with a unique name.
"""
import os
import argparse
...
def parse_args():
    ...
    # UQ: MC Dropout rate (default 0.0 = no dropout, identical to v3 behaviour)
    parser.add_argument("--dropout_rate",    type=float, default=0.0,
                        help="Dropout rate injected into ResBlock.out_layers of UNet. "
                             "0.0 (default) = no dropout (v3-identical). "
                             "Typical MC Dropout value: 0.05.")
...
def main():
    ...
    run = wandb.init(
        ...
        tags=["BBDM", "2.5D", "CT2MRI", "Choo2024", "v4", "MR-histogram", "UQ"],
        config=args_dict
    )
    ...
    # =========================================================================
    # Model
    # v4: same as v3 + dropout_rate parameter for UQ (default 0.0 = no change).
    #     use_context=True, context_dim=128
    #     No HistogramEncoder — raw histogram tensor is the context.
    # =========================================================================
    print("\n" + "=" * 60)
    print("Initializing Model (v4 — UQ edition)...")
    print("=" * 60)
    ...
    model = BrownianBridgeModel(
        ...
        dropout_rate=args.dropout_rate,          # v4: UQ dropout injection
    ).to(device)
    ...
    print(f"Dropout rate       : {args.dropout_rate} "
          f"({'active — MC Dropout enabled' if args.dropout_rate > 0 else 'no dropout (v3-identical)'})")
    print("v4: No HistogramEncoder — raw MR histogram passed as context.")
