"""
Inference script v4 for 2.5D Brownian Bridge Diffusion Model (CT-to-MRI).
UQ edition: supports normal inference AND MC Dropout uncertainty quantification.

Uses v3 dataset (official Choo2024 MR histogram, no HistogramEncoder) and bbdm_model_v4.
Context shape: [D, 128, 3, 1] (avg_histogram expanded) — auto-rearranged to [D, 3, 128]
by attention.py's 4D branch.

The average MR histogram is loaded from the checkpoint (saved during v4 training)
...
Usage:
  # Normal mode (identical to v3):
  python generate_bbdm_ct2mri_v4.py --experiment_name EXP --checkpoint_ver epoch050 ...

  # ISTA mode (identical to v3):
  python generate_bbdm_ct2mri_v4.py --experiment_name EXP --checkpoint_ver epoch050 \\
      --use_ista --num_ISTA_step 1 --ISTA_step_size 0.5 --sub_batch_size 48 ...

  # MC Dropout UQ mode:
  python generate_bbdm_ct2mri_v4.py --experiment_name EXP --checkpoint_ver epoch050 \\
      --mc_dropout --num_mc_samples 10 ...
  Outputs per subject:
    syn_t1_mean_{id}.pt   — mean across MC samples  [D, H, W] in [0,1]
    syn_t1_var_{id}.pt    — variance across MC samples [D, H, W]
"""
import argparse
...
from model.BrownianBridge.bbdm_model_v4 import BrownianBridgeModel  # v4: dropout support
from utils.common import set_random_seed
from utils.mc_dropout import enable_mc_dropout                       # v4: MC Dropout
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate MRI from CT using 2.5D BBDM (v4)")
    ...
    # v4 UQ: MC Dropout options
    parser.add_argument("--mc_dropout", action="store_true",
                        help="Enable MC Dropout UQ mode. Requires model trained with "
                             "--dropout_rate > 0. Replaces nn.Dropout with MCDropout so "
                             "dropout stays active in model.eval().")
    parser.add_argument("--num_mc_samples", type=int, default=10,
                        help="Number of MC Dropout forward passes per subject (default 10). "
                             "Only used when --mc_dropout is set.")
...
def main():
    ...
    # Build model (v4: reads dropout_rate from checkpoint config; defaults to 0.0)
    # context_dim=128 still valid; shape [B,128,3,1]->[B,3,128] via attention.py
    model = BrownianBridgeModel(
        ...
        dropout_rate=config.get("dropout_rate", 0.0),  # v4: load dropout_rate from ckpt config
    ).to(device)
    ...
    print("Model loaded (no HistogramEncoder in v4)")

    # v4 UQ: if MC Dropout requested, replace nn.Dropout → MCDropout (always-on).
    # This must happen AFTER model.eval() to clearly show that eval() alone does
    # not disable MC Dropout.
    if args.mc_dropout:
        dropout_rate = config.get("dropout_rate", 0.0)
        if dropout_rate == 0.0:
            print("WARNING: --mc_dropout is set but checkpoint was trained with "
                  "dropout_rate=0.0. All dropout layers have p=0.0 and are no-ops. "
                  "MC samples will be identical. Re-train with --dropout_rate > 0.")
        enable_mc_dropout(model)
        print(f"MC Dropout enabled: nn.Dropout → MCDropout (always active). "
              f"dropout_rate={dropout_rate}, num_mc_samples={args.num_mc_samples}")
    ...
    if args.mc_dropout:
        inference_suffix = f"mc_dropout_n{args.num_mc_samples}"
        if args.use_ista:
            inference_suffix += f"_ISTA_n{args.num_ISTA_step}_s{args.ISTA_step_size}"
    elif args.use_ista:
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(test_set)), desc="Generating"):
            ...
            if args.mc_dropout:
                # ---- MC Dropout UQ: run num_mc_samples forward passes ----
                # MCDropout (always-on) ensures each pass uses a different
                # random dropout mask, even inside torch.no_grad().
                mc_samples = []
                for mc_idx in range(args.num_mc_samples):
                    if args.use_ista:
                        mri_syn_i = reconstruct_3d_ista(
                        ...
                        mri_syn_i = reconstruct_3d_normal(
                            model, ct, avg_histogram, device, args.use_bf16, target_size
                        )
                    mc_samples.append(mri_syn_i)  # each: [D, H, W] in [0, 1]

                # Stack → [num_mc_samples, D, H, W]
                mc_stack = torch.stack(mc_samples, dim=0)
                mri_mean = mc_stack.mean(dim=0)    # [D, H, W]
                mri_var  = mc_stack.var(dim=0)     # [D, H, W]  (unbiased by default)

                # Save UQ outputs
                torch.save(mri_mean, output_dir / f"syn_t1_mean_{subject_id}.pt")
                torch.save(mri_var,  output_dir / f"syn_t1_var_{subject_id}.pt")

            else:
                # ---- Standard single-pass inference (identical to v3) ----
                if args.use_ista:
            ...
            # Save GT MRI and CT (same in both modes)
